...
class EventsTest(SoupTest):
    ...
    def test_create_event_success(self):
        ...
        with scopes_disabled():
            ...
            assert ev.date_from == berlin_tz.localize(datetime.datetime(2016, 12, 27, 10, 0, 0)).astimezone(datetime.timezone.utc)
            assert ev.date_to == berlin_tz.localize(datetime.datetime(2016, 12, 30, 19, 0, 0)).astimezone(datetime.timezone.utc)
            assert ev.presale_start == berlin_tz.localize(datetime.datetime(2016, 11, 1, 10, 0, 0)).astimezone(datetime.timezone.utc)
            assert ev.presale_end == berlin_tz.localize(datetime.datetime(2016, 11, 30, 18, 0, 0)).astimezone(datetime.timezone.utc)
